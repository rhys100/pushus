...
### 2026-07-11n (sign-in email template polish)

- Redesigned `magic_link.html` to match app tokens: dark `#0a0a0d` / `#16161c` surfaces, `#ff6b35` accent, large mono OTP block, table layout for Gmail. Plain-text template puts the code alone between rules so it is not buried in a paragraph.
- Rhys must re-paste HTML into Supabase **Authentication → Email Templates → Magic Link** (Body HTML), or run `npm run auth:push-email-template`.
...
- **Reported fixes.** (1) Stale push reminder ("do 10" from an earlier day) — root cause was a notification left in the tray, not the plan divergence I first suspected (adversarial verification refuted that by temporal impossibility). Fix: `sw.js` closes all `pushus-reminder` siblings on tap + carries `localDate`/`sentAt`; new `useDismissStaleReminders` clears reminders on foreground; edge `send-push-reminders` self-dates the payload and sends with `{ TTL: min(interval,360)*60, urgency:'high', topic }`. (2) Edge `.select()` was missing 4 calibration columns so the notification target could diverge below the app for calibrated users — added them. (3) Availability copy: `injured`/`sub_out` are mechanically identical (same RPC, streak walker ignores status) — unified the card copy + symmetrized the toasts.
- **Polish pass.** Ran a 10-lens adversarially-verified audit (120 findings) then batch-fixed 44 of 47 High/Medium across 7 disjoint-file agents + a cleanup agent: logger post-bank input-freeze + reduced-motion, overage count freeze, max-set exit, nose-tap discard confirm, narrowed bank-mutation invalidation (per-month + covering leaderboard periods), lazy-loaded logger sheets (main chunk 218.8→211.3 kB), friendly create/join/onboard errors, Achievements/Mates error states, wizard zero-day guard, min-h-[100dvh] + focus rings + dead-token fixes.
- **Still open (flagged to Rhys, NOT done):** mate-link-breaks-for-non-members (High — magic-link auth rewire, needs runtime test), leave/switch challenge team (needs a mutation, maybe RLS), `cn`→tailwind-merge (dep + app-wide). Deferred: reactions feed→waterfall (needs RPC change), 63 low-severity nitpicks, larger a11y (dialog focus-trapping).
- Verified: `tsc` 0 err, eslint 0 err, 440 tests, prod build clean. Notification edge fn + SW changes need a deploy to take effect.

### 2026-07-09 (feature — edit/log yesterday, XP-on-bank feedback, "How it works" page)

- **Root of Rhys's "gamification got lost" report:** nothing was overwritten — 0031 (XP trigger + achievement catalog + unlocks) is intact and routed. Two real causes: (1) there's **no per-bank XP feedback** — badge-unlock toasts fire on the early-milestone rush then go quiet, so banking *feels* unrewarded; (2) migrations **0031–0041 may not be deployed** to the live DB (empty catalog ⇒ no unlocks). Also verified the suspected **double-XP is not a bug**: `bank_pushups` calls `award_xp_for_entry` inline *and* 0031 adds an AFTER-INSERT trigger, but `award_xp` inserts `ON CONFLICT (source_type, source_id) DO NOTHING` and `refresh_user_xp_total` recomputes the total from the ledger, so it reconciles to one row / correct total.
- **Shipped — log & edit yesterday.** Members were hard-blocked to "same-day edit only" (`update_pushup_entry`, 0002) even though `bank_pushups` already honours `backdate_policy` (`today_yesterday`). Mig **0041** relaxes member edits to `is_backdate_allowed(group, logged_for)` (today+yesterday by default; older locked; admins bypass). Delete stays same-day per Rhys. Client: `useBankPushups` takes an optional `loggedFor` (passes `p_logged_for`, resolves optimistic day-cache to the target date, invalidates on backdated success since it can fall outside the current leaderboard period); new `AddSetForDay` in My log shows a "forgot a set on {day}?" adder for yesterday; `DayEntriesList` now gates Edit (today+yesterday) / Delete (today) via `lib/entryEditWindow.ts` (+ unit tests) and shows a 🔒 lock note on older days.
- **Shipped — XP feedback.** Bank toasts now read "N push-ups banked · +N XP" (push-ups only; custom activities don't earn XP). Directly answers "no points today."
- **Shipped — "How PushUS works" page** (`/settings/how-it-works`, linked in Settings → About): honour system, today/yesterday window, 1 push-up = 1 XP, streaks/rest-days/freeze, badges, boards, reactions/mates/challenges, injured/away. Copy matches the new edit rule.
- **Adversarial review (9-agent workflow) → fixed 4 findings:** (1) *security* — relaxing member edits to yesterday let a member shrink an admin-**rejected**/pending yesterday entry back under the cap, flipping `review_status`→`none` so it silently counts again (admin can't re-reject). 0041 now blocks member edits of `pending_review`/`rejected` entries (also closes the pre-existing same-day variant; admins bypass). (2) *cache* — a bank only invalidated `['xp']` when a badge unlocked, so the Badges XP total sat stale ~30s after a normal bank; `useBankPushups.onSuccess` now always invalidates `['xp',group,user]`. (3) *UX* — `EntryRow` edit/delete failures reverted silently; now toast `getErrorMessage`. (4) *copy* — streak-freeze rule text overstated flexibility; reworded to "protect yesterday" matching the actual UI. Left (UNCERTAIN, latent): client `entryEditWindow` hardcodes `today_yesterday` and ignores a group's `backdate_policy` — no shipped group uses a non-default policy, and the new error toasts now explain a server rejection if one ever does; deferred threading the real policy into the Group type.
- **Also this session:** fixed a **Board text bug** (day view — a member's `10/10` score clipped to `10/1` on narrow phones because the name column was fixed `w-24 shrink-0`; now `min-w-0 max-w-24 truncate` so the score always shows). Expanded the **How-it-works page** with a beginner "reps & sets in 20 seconds" primer (a real invited user was confused by "2 sets of 8" — didn't know rep/set terms) + fuller gamification (XP/streak-flame/badges/board framing).
- **Next / not done here:** run `supabase db push` for 0031–0041 and redeploy edge fns (deploy has lagged since the July push) — confirm live XP total climbs and badges unlock. Consider an XP/level chip on Today if the toast isn't enough.


- **CSS/animation perf audit: clean.** All 21 `@keyframes` in `styles/motion.css` animate only transform/opacity (no width/height/top/left/margin/padding → no reflow); no `transition:`/`animation:` on layout props. Follows the AGENTS.md performance rules.
- **Extended the dead-export check to `scripts/` + root `functions/`** (my earlier scan missed those — a changelog formatter could plausibly be release tooling). Two candidates verified 0-refs across the *entire* repo (src+tests+supabase+e2e+scripts+functions): `toLegacyInstallStatus` (+ its paired `@deprecated LegacyPwaInstallStatus` type) in `lib/pwaInstallStatus.ts`, and `formatChangelogReleaseSection` in `lib/releaseVersion.ts` (not wired into any release script). Removed both — pure leftover utilities, non-careful-area, zero behaviour impact. Then also removed `isDinkSupported` (`lib/dinkSound.ts`) and `pushSupportAllowsEnable` (`lib/notifications/registerPush.ts`) — same 0-refs-everywhere verification, non-careful predicates (`getAudioContext`/`PushSupportStatus` stay used by their siblings, so no orphaning). That exhausts the clearly-safe removals: the ~14 remaining flagged candidates are all training-lib/logger careful areas + possibly-intended admin hooks (`useRevokeBadge` etc.) — holding those pending a human OK, not removing autonomously. tsc -b + lint (0 errors) + 436 tests + prod build all green.

### 2026-07-09 (perf loop — dead-export scan; remove 2 orphaned dock helpers)

- **Ran a strict dead-export scan** (exported functions whose name appears exactly once — only its definition — across src+tests+supabase+e2e; a first file-count-based scan was too noisy with false positives from intra-file use, test-only use, and edge-fn mirrors, so I discarded it). Surfaced ~20 genuinely-unreferenced exported functions.
- **Removed 2** that pass 9 had just orphaned: the `@deprecated` `getPwaInstallDockVisible` and `subscribePwaInstallDockVisibility` aliases in `lib/pwaInstallDockVisibility.ts` (both delegated to the live `getInstallOpenAppDockVisible` / `subscribeBottomDockPromptVisibility`; `setPwaInstallDockVisible` stays — still used). 0 references repo-wide now.
- **NOT removed, flagged for a deliberate review** (bigger call than a 4-min loop pass, and several sit in AGENTS.md "careful areas"): `ringAngleToRawAngle` (circularCounter — careful), `isDinkSupported`, `pushSupportAllowsEnable`, `toLegacyInstallStatus`, `formatChangelogReleaseSection`, and training-lib functions `estimateWeeklyVolume`, `recommendFromWizardConservative`, `recommendFromWizardDraft`, `suggestProgression`, `sorenessHoldsProgression`, `computeTrustedVolumeAnchor`, `volumeContextFromStoredAverage`, `previewExplanationForContext`, `referencePeakAtFullBlock`, `formatWizardPlanSummaryParagraph`, `shouldPromptPlanMaxUpdate`; plus unused hooks `useGroupWriteAccess`, `useGroupAwardedBadges`, `useRevokeBadge`, `useTodayEntries`. All appear unreferenced but removing plan-engine/logger exports warrants a human OK. tsc -b + lint (0 errors) + 436 tests + prod build green (build needed a re-run past a transient Dropbox FS glitch — the 3rd this session).

### 2026-07-09 (perf loop — remove dead usePwaInstallDockVisible hooks)

- **Dead code:** `usePwaInstallDockVisible` was exported from **two** modules yet imported by **nobody** (verified across src/tests/e2e). One (`useBottomDockPromptVisible.ts`) was an explicitly `@deprecated` alias delegating to the live `useInstallOpenAppDockVisible`; the other (`usePwaInstallPrompt.ts`) was an orphaned `useSyncExternalStore` reader. The install-dock flow works without them — `setPwaInstallDockVisible('install', …)` writes the state (live, in usePwaInstallPrompt's main hook) and `useInstallOpenAppDockVisible` reads the aggregate (live, used by `usePushNotificationPrompt`). Removed both dead exports and the now-orphaned `getPwaInstallDockVisible` / `subscribePwaInstallDockVisibility` imports; `usePwaInstallDockVisible` now has 0 references repo-wide. Also measured this pass: the production bundle is already excellently code-split (~60 granular per-page/per-hook chunks, 276 kB gzip total with most lazy, 67 kB main) — no bundle lever; UI primitives aren't `React.memo`'d so inline option-arrays defeat no memo. tsc -b + lint (0 errors) + 436 tests + prod build all green (build needed a re-run past a transient Dropbox FS glitch, not a code issue).

### 2026-07-09 (perf loop — dedupe getPlatform via existing readPwaInstallPlatform)

- **`getPlatform` was duplicated** (byte-identical) in `usePwaInstallPrompt` and `usePwaOpenAppPrompt` — each a local `try { getPwaInstallPlatform(navigator.userAgent, navigator.platform, navigator.maxTouchPoints) } catch { 'other' }` wrapper. `lib/pwaInstallStatus.ts` already exports **`readPwaInstallPlatform`** which is the same wrapper plus an SSR `typeof navigator === 'undefined'` guard (a no-op in these client hooks). Both hooks already imported from `pwaInstallStatus`, so swapped `useMemo(getPlatform, [])` → `useMemo(readPwaInstallPlatform, [])`, deleted the two local `getPlatform` copies, and dropped the now-unused `getPwaInstallPlatform` + `type PwaInstallPlatform` imports from `pwaInstallPrompt`. Behaviour-identical (the SSR guard never fires here). Left alone (not clean dups): `friendlyAuthError` (LoginPage vs AuthCallbackPage — deliberately different OAuth-failure copy), `dayOfWeekFromIso` (UTC-only vs timezone-aware — different semantics, streak logic is a careful area). tsc -b + lint (0 errors; the one `platform` exhaustive-deps warning is pre-existing on HEAD) + 436 tests + prod build all green.

### 2026-07-09 (perf loop — scoreChallenge O(P×E)→O(P+E); dedupe GroupPage members query + shared helpers)

- **`clamp` ×3 and `roundReps` ×3** — identical file-local helpers copy-pasted across `planEngine.ts`, `trustedVolume.ts`, and `volumeCalibration.ts` (which already form ES-module cycles among themselves). Extracted into a new **leaf** module `lib/training/numeric.ts` (imports nothing → can't add a cycle) and rewired all three importers; removed 6 local copies. Pure move, behaviour-identical, verified single-source. tsc -b + lint + 436 tests + prod build green.
- **Three more byte-identical duplicated helpers consolidated** (found via a repo-wide scan for functions defined in 2+ files): `statusVariant` (challenge status→Badge tone) was copy-pasted in `ChallengesPage` + `ChallengeDetailPage` → moved to `challenges.ts` beside `challengeStatus`/`ChallengeStatus`; `easeOutCubic` was duplicated in `ProgressChart` + `useCountUp` → moved to `lib/motion.ts` (the shared-motion module); `formatEntryTime` was duplicated in `DayEntriesList` + `CustomDayEntriesList` → moved to `lib/repHistoryFormat.ts` (which already imports `format`/`parseISO`), which also let both entry-list files drop their now-unused `date-fns` import. Each is a pure move, behaviour-identical, verified single-source afterwards. (Left for a dedicated pass: the `clamp` ×3 and `roundReps` ×3 duplicates in `src/lib/training/` — they'd want a new shared numeric module.) tsc -b + lint + 436 tests + prod build all green.
- **Duplicate `dayTypeBadgeVariant` helper** lived in both `DayProgressCard` and `TrainingWizard` — byte-identical dayType→Badge-variant mapping (the two return-type unions were even ordered differently, exactly the kind of drift that duplication invites). Consolidated into `planEngine.ts` beside the existing `getDayTypeDisplayLabel` day-type display helpers; both components now import it. Pure move, behaviour-identical. Also audited clean this pass: `useRepHistory`/`RepHistoryPanel` (memoized aggregation, per-month bounded fetch, push-up vs custom queries mutually disabled), every `pushup_entries` select (minimal columns — `logged_for,count` etc., zero `select('*')`), and `useStreaks` (bounded 70-day window, 3 reads in parallel via `Promise.all`). tsc -b + lint + 436 tests + prod build all green.
- **`GroupPage` had its own inline copy of the group-members query** — a duplicate `fetchMembers` + `useQuery({ queryKey: ['group-members', groupId], ... staleTime: 30_000 })` that reimplements the existing `useGroupMembers` hook byte-for-byte (same `list_group_members` RPC, same key) but with a *different* staleTime (30s vs the hook's 60s). Same key already deduped the network, but it was ~15 lines of drift-prone duplication and two observers with mismatched staleness on one key. Replaced with `const membersQuery = useGroupMembers(groupId)` and dropped the now-unused `useQuery`/`supabase` imports. Behaviour identical (same data shape, and member changes still force-refetch via the existing `invalidateQueries(['group-members', groupId])` in `GroupAdminSettings`/`useMemberAlias`); members now cache for 60s consistently everywhere. tsc -b + lint + 436 tests + prod build all green.
- **`scoreChallenge` (`src/lib/challenges.ts`) re-scanned every challenge entry once per participant** — `entries.reduce(...)` nested inside `participants.map(...)`, i.e. O(participants × entries). Bucketed entries into a `Map<user_id, entries[]>` once up front so each participant only reduces its own rows → O(participants + entries). Behaviour is byte-identical (the per-participant date-window filter using each participant's `countedFromIso`/`endIso` stays inside the loop). Runs inside a `useMemo` in `ChallengeDetailPage`, so it only fires on entries/participants change, not per render. **Honest impact:** at realistic sizes (a 30-day group challenge ≈ 20 participants × ~2k entries ≈ 40k comparisons) this was already sub-millisecond, so this is a scalability/clarity win, not a measured current bottleneck. Verified: `tests/unit/challenges.test.ts` (11 tests, incl. late-joiner scoring) pass unchanged; tsc + lint clean. Audited but left alone: the Board is aggregated/ranked server-side (`leaderboard_total`/`leaderboard_metric` RPCs — client just maps rows O(n)); `scoreTeams` is O(teams × memberships) but teams = 2; `timezoneOptions()` is a 16-item curated list (per-render recompute is negligible).

### 2026-07-09 (design-audit loop — user-requested Log/notification polish + rolling QA batch)

- **User-requested changes (Log screen + reminders):** (1) Push-reminder settings copy now states reminders only fire during chosen hours and **stop for the day once all sets are banked** — the `send-push-reminders` edge fn already skips `bankedToday >= todayTarget`, so this was a copy gap, not a behaviour change. (2) Banked toast redesigned — Undo is now a bordered pill and the close button a 36px round target (`Toast.tsx`). (3) `TodayPage` ring cluster now centres in the space below a pinned activity switcher (drops the ring into the thumb zone) and the logger carries `-mb-5` to absorb the ring SVG's ~23px internal whitespace so `+10` hugs it. (4) Same `-mb-5`/`mt-2` spacing ported to `GuestPage` (it kept its own copy of that layout).
- **Challenge creator:** "Create challenge" wrapped to two lines (base `Button` `px-6` inside a two-up `flex-1` row) → `whitespace-nowrap px-3`; the 4-up intensity grid tightened (`gap-1.5 px-1 whitespace-nowrap`) so "Moderate" fits; every intensity tap now fires `tapHaptic`.
- **Oversights found combing the app:** guest-mode delete now uses the app-wide two-tap confirm (`confirmDeleteId` + 4s auto-reset) — was one-tap-instant in the one mode where reps are device-only; trial banner + billing panel say "ends today" not "ends in 0 days" (`daysUntil` clamps to 0); `SegmentedControl` dropped its blanket `capitalize` (was title-casing "Best set"/"Most improved"/"My log" — audited all 5 usages first); login footer + `buildInviteMessage` + share title now use `appConfig.name` (were hardcoded "PushUS"); team-join warning arming scoped to the tapped team (`ChallengeDetailPage`, was a page-wide bool that could skip a team's warning); achievement lifetime bars got `role="progressbar"` + a 4% min-fill; `SetEffortSheet` renders its per-option hints (matching the soreness sheet); training-wizard selectors got `active:scale` press feedback + the max-clean slider shows its 1–60 endpoints; plus achievements empty-state copy, nudge/challenge-card overflow guards, leaderboard name width, list tap-target floor (`min-h-9`), and a no-plan-card capitalisation.
- **Browser-verified** (Chrome via `/dev/preview` + `/login` + `/guest`): ring SVG whitespace measured at 23px (confirms `-mb-5`); `SegmentedControl` computed `text-transform: none`; login header + footer both resolve `appConfig.name`; the milestone toast shows the pill Undo + enlarged ×; guest two-tap delete driven end-to-end (tap 1 → red "Confirm?", set count unchanged; tap 2 → removed; 4s auto-reset). `tsc -b` + lint + 436 unit tests green throughout.

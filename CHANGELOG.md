...
- **Toast and banner readability:** success, warning, danger, and accent notifications now use opaque surfaces instead of 10–30% transparent tints, so copy stays legible when toasts float over page content (e.g. invite copy confirmation on Settings) the "hold centre for nose reps" teaching hint now sits below the Bank Push-ups button at the bottom of the Log page, and no longer hides when you start dragging (which was shifting the ring upward in the centred layout)

...
- **iOS PWA sign-in persistence:** email links open in Safari, whose storage is isolated from the Home Screen app. Sign-in emails now include a 6-digit code that is entered and verified inside PushUS, creating the session in the PWA itself. The unreliable Cache Storage token bridge was retired and its stored refresh tokens are deleted

...
- **iOS PWA sign-in persistence:** email links open in Safari, whose storage is isolated from the Home Screen app. Passwordless sign-in now uses a 6-digit code entered and verified inside PushUS, creating the session in the PWA itself. Code-only emails avoid link scanners consuming the credential. Hosted template can be pushed with `npm run auth:push-email-template`. The unreliable Cache Storage token bridge was retired and its stored refresh tokens are deleted per browser context

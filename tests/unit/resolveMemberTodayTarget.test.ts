...
import { getDefaultPlan, getTodayPrescription } from '../../src/lib/training/planEngine'
import { resolveMemberTodayTarget } from '../../src/lib/training/resolveMemberTodayTarget'

const SYDNEY = 'Australia/Sydney'

describe('resolveMemberTodayTarget', () => {
  it('uses default plan when row is missing', () => {
    const today = '2026-06-29'
    const result = resolveMemberTodayTarget(null, today, SYDNEY)
    const expected = getTodayPrescription(getDefaultPlan(), today, SYDNEY)

    expect(result).toEqual({
      target: expected.target,
      isRestDay: expected.isRestDay || expected.target === 0,
    })
  })

  it('uses default plan when wizard is not completed', () => {
    const today = '2026-06-29'
